...
function dimInfinityMult() {
  return Currency.infinitiesTotal.value.times(0.1).plus(1);
}
function chargedDimInfinityMult() {
  return 1 + Math.log10(Math.max(1, Currency.infinitiesTotal.value.pLog10())) * Math.sqrt(Ra.pets.teresa.level) / 300;
}
...
export const infinityUpgrades = {
  totalTimeMult: {
    ...
    effect: () => Math.pow(Time.totalTimePlayed.totalMinutes / 2, 0.15) / 2,
    ...
    charged: {
      ...
      effect: () => 1 +
        ...
        Math.pow(Ra.pets.teresa.level, 0.5) / 300,
  ...
  buy10Mult: {
    ...
    charged: {
      ...
      effect: () => 1 + Ra.pets.teresa.level / 400,
  ...
  thisInfinityTimeMult: {
    ...
    effect: () => Decimal.max(Math.pow(Time.thisInfinity.totalMinutes / 4, 0.25) / 2, 1),
    formatEffect: value => formatX(value, 2, 2),
    charged: {
      description:
      ...
      effect: () => 1 +
        ...
        Math.sqrt(Ra.pets.teresa.level) / 300,
      formatEffect: value => formatPow(value, 4, 4)
    }
  },
  unspentIPMult: {
    ...
    effect: () => Currency.infinityPoints.value.dividedBy(2).pow(1.5).plus(1).pow(0.5),
    formatEffect: value => formatX(value, 2, 2),
    charged: {
      description: "Multiplier to 1st Antimatter Dimension based on unspent Infinity Points, powered by Teresa level",
      effect: () => Currency.infinityPoints.value.dividedBy(2).pow(Math.sqrt(Ra.pets.teresa.level) * 1.5).plus(1).pow(0.5),
      formatEffect: value => formatX(value, 2, 2)
    }
  },
  dimboostMult: {
  ...
  ipGen: {
    ...
    description: () => `Passively generate Infinity Points ${formatInt(20)} times slower than your fastest Infinity`,
    ...
    formatEffect: value => {
      ...
      return `${format(value, 2)} every ${Time.bestInfinity.times(20).toStringShort()}`;
    },
    charged: {
      description: () =>
  ...
  ipOffline: {
    ...
    effect: () => (player.options.offlineProgress
      ? player.records.thisEternity.bestIPMsWithoutMaxAll.times(TimeSpan.fromMinutes(1).totalMilliseconds / 4)
      : DC.D0),
  ...
  ipMult: {
    ...
    description: () => `Multiply Infinity Points from all sources by ${formatX(1.5)}`,
